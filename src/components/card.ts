  ...
  getOwnerButtonsHtml() {
    ...
    if (this.currentUser === this.createdBy) {
      ownerButtons = html` <div class="text-center str-card-buttons-container">
        <str-form-btn
          buttonText="${this.updatebuttonText}"
          isFormButton
          isCardButton
          aria-label="Update Product"
          @click="${(e) => {
            e.stopPropagation();
            this.onProductOperation(EVENTS.UPDATE_PRODUCT);
          }}"
        >
        </str-form-btn>
        <str-form-btn
          buttonText="${this.deletebuttonText}"
          isFormButton
          isCardButton
          aria-label="Delete Product"
          @click="${(e) => {
            e.stopPropagation();
            this.onProductOperation(EVENTS.DELETE_PRODUCT);
          }}"
        >
        </str-form-btn>
      </div>`;
  ...
  render() {
    ...
    return html`
      <button
        type="button"
        class="card"
        @click="${this.onProductOperation.bind(this, EVENTS.VIEW_PRODUCT)}"
      ...
      >
        ...
        <div class="container card-details">
          <h4 class="text-center card-font-medium card-header-margin">
            <b>${this.productName}</b>
          </h4>
          <p class="text-center card-font-small card-details-margin">
            ${this.productDescription}
          </p>
          <p class="text-center card-font-small card-details-margin">
            <b><span>&#8377; </span>${this.productPrice}</b>
          </p>
        </div>
        ${ownerButtons}
      </button>
